...
export default function AIModal({
  ...
  const handleAISubmit = async () => {
    ...
    setIsAILoading(true);
    try {
      const [analysisResult, servicesResult] = await Promise.all([
        analyzeNeed(aiQuery),
  ...
  const handleGeneratePaths = async () => {
    ...
    try {
      ...
      alert(
        "Erreur lors de la génération des chemins de services. Veuillez réessayer."
      );
    } finally {
      setIsGeneratingPaths(false);
    }
          ...
          {(needAnalysis || suggestedServices.length > 0) && (
            <div className="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              <p className="text-xs text-blue-600 dark:text-blue-300 flex items-center">
                ...
                Une nouvelle analyse effacera l'analyse actuelle et tous les
                chemins de services générés.
              </p>
            </div>
          )}
        </div>
            ...
            {servicePaths.length > 0 ? (
              <div className="space-y-4 max-h-96 overflow-y-auto">
                {servicePaths.map((path, pathIndex) => (
                  <div
                    key={path.id}
                    className="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4"
                  >
              ...
              <div className="bg-gradient-to-r from-orange-50 to-yellow-50 dark:from-orange-900/20 dark:to-yellow-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
                <div className="flex items-center text-orange-700 dark:text-orange-300">
                  ...
                  <div>
                    ...
                    <p className="text-sm">
                      L'IA n'a trouvé aucune offre correspondant exactement à
                      votre besoin dans notre catalogue actuel. Vous pouvez :
                    </p>
                    <ul className="text-sm mt-2 space-y-1">
                      <li>
                        • Reformuler votre demande de manière plus générale
                      </li>
